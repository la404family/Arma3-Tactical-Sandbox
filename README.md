# Tactical Sandbox — A Fully Customizable Mission Launcher

> Tactical Sandbox is a mission launcher designed to be tailored exactly the way you want.
> Experience the ultimate freedom in mission planning and execution.

## ⭐️ KEY FEATURES

**Universal Compatibility**
The system automatically adapts to your installed Mods and DLCs.

**Dynamic Gameplay**

- **Detailed Briefings:** All missions include specific task instructions.
- **Infinite Replayability:** Every objective takes place in a different location with over **50+ possible areas per mission**
- **⚠️ Permadeath System:** Survival truly matters. If you fall, the consequences are real.
Revive or medical overhaul mods don’t fully adapt to this mission, but you can switch to a nearby AI teammate if your character dies.

## 🏢 THE HEADQUARTERS

![Headquarters](Slide14.JPG)

Prepare for your mission in a fully interactive HQ featuring specialized zones:

- **🔫 1. Virtual Arsenal:** Choose your uniforms, patches, weapons, and gear without restriction.
- **⛅ 2. Weather & Time Room:** Take full control over the time of day and all weather conditions (Fog, Rain, Storms, Clear Skies).
- **🗺️ 3. Mission Selector:** Choose from multiple dynamic mission types suited to your mood.
- **🪖 4. Recruitment Center:** Build and customize your AI squad (*"Brothers in Arms"*). Add or remove soldiers at any time.
New units appear seamlessly, and friendly forces will wear the uniforms from your loaded mods.
Allies appearing in missions (via helicopter or others) will wear the server players' outfits (it's better for immersion!).
- **🎯 5. Enemy Selector:** Customize opposing forces. *Note: Default vanilla units are included, but you are free to use **ANY** OPFOR or Independent faction.*
You must modify the enemies before launching a mission (the selected enemies will be the mission's enemies).
- **🚙 6. Garage:** Select your transport.
Choose from a wide variety of ground vehicles and helicopters.

## 🌍 SUPPORTED LANGUAGES

- 🇬🇧 English
- 🇫🇷 Français
- 🇩🇪 Deutsch
- 🇪🇸 Spanish
- 🇮🇹 Italiano
- 🇷🇺 Русский
- 🇵🇱 Polski
- 🇨🇿 Česky
- 🇹🇷 Türkçe
- 🇨🇳 中文 (Simplified & Traditional)
- 🇯🇵 日本語
- 🇰🇷 한국어
- 🇵🇹 Português

## 💻 SOURCE CODE & DEVELOPMENT

This project is Open Source.
Feel free to follow my work or check the code!

## ❤️ PLEASE !!

**Follow me on GitHub:** 
**Drop a Star on the project:**

# 🎯 Mission Overview — Tactical Sandbox

---

## 🏃‍♂️ Task 1 — Manhunt
Track down fugitives attempting to escape the island by sea.  
Neutralize all targets before they reach their boats.  
⚠️ Mission fails instantly if even one fugitive escapes.

---

## 🎯 Task 2 — Assassination
Locate three enemy officers — only **one** carries vital intel.  
Identify, eliminate, and recover classified documents from the correct body.  
Stealth or brute force — your call.

---

## 💣 Task 3 — Cargo Destruction
Enemy ammunition caches are hidden and guarded by infantry.  
Find and destroy them all.  
Allied air support will assist with target spotting.

---

## 🕶️ Task 4 — Hostage Extraction
A hostage is held in one of three possible locations.  
Eliminate guards without harming the civilian.  
Escort the hostage to the extraction point for helicopter pickup.  
**Failure if the hostage dies.**

---

## 👥 Task 5 — Civilian Zone & Bomb Defusal
Two bombs are armed in a populated area.  
Civilians are everywhere — but traitors hide among them.  
Disarm the explosives while avoiding civilian casualties.  
Time is limited.

---

## 🚁 Task 6 — Allied Rescue
An allied helicopter has crashed behind enemy lines.  
Secure the crash site and aid the wounded survivors.  
Defend against enemy waves.  
At least one survivor must live.

---

## 📡 Task 7 — Radar Destruction
A mobile enemy radar is scanning the skies.  
Find the correct site and destroy the installation.  
Heavy infantry presence — strike hard and fast.

---

## 📊 STEAM WORKSHOP STATS

![Steam Stats](ReadmeStat.png)

This project has been well-received by the Arma 3 community! Here is a snapshot of the current performance on the Steam Workshop:

- **👥 Unique Visitors:** +1000 players have checked out the mission page.
- **✅ Subscribers:** +200 players have subscribed and installed the mission.
- **⭐ Favorites:** +40 players have added it to their favorites list.
- **👍 Positive Ratings:** +82% Positive ratings (9 positive out of 11 total).
